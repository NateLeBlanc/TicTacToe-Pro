...
function player.PlayerMove(gameBoard, activePiece, GridOptions, MouseObj)
    if MouseObj.button ~= 1 then return gameBoard, activePiece end

    if not activePiece then
        print("Error: currentPlayer is nil in PlayerMove.")
    ...
    for row = 1, GridOptions.gridSize do
        for col = 1, GridOptions.gridSize do
            ...
            if MouseObj.x >= cell.x and MouseObj.x < cell.x + GridOptions.cellSize and
                ...
                if cell.placedPiece == "" then
                    cell.placedPiece = activePiece
                    nextPlayer = (activePiece == ActivePiece.X) and ActivePiece.O or ActivePiece.X
                end
            end
        end
    end
...
isWinningLine = function(gameBoard, coords)
    local firstValue = gameBoard[coords[1][1]][coords[1][2]].placedPiece
    if firstValue == "" then return false end

    for i = 2, #coords do
        local row, col = coords[i][1], coords[i][2]
        if gameBoard[row][col].placedPiece ~= firstValue then
            return false
        end
    end
    return true
